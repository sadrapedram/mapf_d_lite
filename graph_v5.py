...
class TrafficSimulation:
    def __init__(self, place="Vake, Tbilisi, Georgia", num_cars=50, speed=10.0):
        ...
        # NEW: Add wait_count to track how long each car has been waiting
        self.wait_count = [0] * num_cars
    ...
    def update(self, frame):
        """Update the simulation state and visualization for each frame with MAPF."""
        if not self.running:
        ...
        self.traffic_flow = temp_traffic
        
        # NEW: Determine intended moves and resolve conflicts
        intended_moves = {}  # edge -> list of cars wanting to move to it
        for i in range(self.num_cars):
            if (self.car_active[i] and 
                self.current_edges[i] is None and 
                self.path_index[i] < len(self.car_paths[i]) - 1):
                current_node = self.car_positions[i]
                next_node = self.car_paths[i][self.path_index[i] + 1]
                edge = (current_node, next_node)
                if edge in self.city_graph.edges:
                    intended_moves.setdefault(edge, []).append(i)
        
        # NEW: Resolve conflicts based on priority (lowest index = highest priority)
        allowed_cars = set()
        for edge, cars in intended_moves.items():
            if cars:
                highest_priority_car = min(cars)  # Lowest index has highest priority
                allowed_cars.add(highest_priority_car)
                # Make other cars wait
                for car in cars:
                    if car != highest_priority_car:
                        self.wait_count[car] += 1
                        # Replan if waiting too long (e.g., 10 frames)
                        if self.wait_count[car] > 10:
                            new_path = self._d_star(self.car_positions[car], self.car_ends[car])
                            if new_path:
                                self.car_paths[car] = new_path
                                self.path_index[car] = 0
                                self.wait_count[car] = 0
        
        # Update car positions
        for i in range(self.num_cars):
            if self.car_active[i]:
                # NEW: Only move if allowed (highest priority for the edge)
                if self.current_edges[i] is None and i in allowed_cars:
                    current_node = self.car_positions[i]
                    next_node = self.car_paths[i][self.path_index[i] + 1]
                    edge = (current_node, next_node)
                    self.current_edges[i] = edge
                    self.progress[i] = 0.0
                    self.wait_count[i] = 0  # Reset wait count when moving
                ...
                if self.current_edges[i]:
                    ...
                    if self.progress[i] >= 1.0:
                        ...
                        if self.path_index[i] >= len(self.car_paths[i]) - 1:
                            self.car_active[i] = False  # Reached destination
                        else:
            ...
            # Visualization
            if self.car_active[i] and self.current_edges[i]:
                edge = self.current_edges[i]
